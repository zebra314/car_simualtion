...
// Joystick control
function loadJoystickController() {
  let card = newCard();
  let topic_name = "/pollo/cmd_vel";
  let message_type = "joystick";
  let joystickController = Viewer.getDefaultViewerForType(message_type);
  
  try {
    subscriptions[topic_name] = {
      topicType: message_type,
      viewer: new joystickController(card, topic_name, message_type)
    };
  } catch(e) {
    console.log(e);
    card.remove();
